...
document.addEventListener("DOMContentLoaded", function () {
	...
	// txtFind.addEventListener('click', changeToEnglish); // IME를 영문으로 바꾼다.
...
/** 그 라인으로 가고, 상/하단 바에, 어느 함수 내인지도 업데이트 한다. 
gotoLine2 에서 기능이 더 있는 함수. */ 
function gotoLine3(line1) {
	scrollToLineNumber('code1', line1);
	// 라인에 하이라이트 준다
	selectLine('code1', line1);	
...
/** 상단 함수 이름 바, 하단 함수 라인 바를 업데이트한다. */ 
function updatePrevBars() {
...
// LINK 버튼 생성 (버튼 그림은...  https://postimg.cc/cKYsV2V0 )
function generateLinkButton(link1, divName) {
	const button = document.createElement("button");
	const maxLength = 20;
	const link2 = (link1 == null) ? '12345No Link' : link1;
...
function constructSelectBox() { // !!HTML_call
  ...
  // 호출도(그림 도??) 함수 리스트의 클릭 이벤트 작성. ????????????????
	selectElement.addEventListener('click', function() {
		...
		console.log("호출 안되나");	// 아직 대체 못함(generateSelectOptions()를..)
...
/** 옵션리스트를 만든다. 그리고 리스트 항목들의 이벤트 리스너도 포함됨. */
function generateSelectOptions(defOrLis) { // !!HTML_call
  ...
  let defs00 = null;
	...
	// 출력예: https://postimg.cc/94jfhb9d 
	console.log("🍀🍀----------------🍀🍌");	
	let printer1 = [];
  defs00.forEach(item => {
    printer1.push(item.functionName); // 함수리스트 프린터용(콘솔에)
  ...
  selectElement.addEventListener('change', function() {
	...
	selectElement.addEventListener('click', function() {
		...
		gotoLine3(lineNum);	// 함수정의로 스크롤 한다. (scrollToLineNumber()의 줄 이동만으로는 정확치 않으므로 해당 부분을 SELECTION을 해줌)
		...
		const currentLine = getCaretLineNumber(textarea1);
		/** A ROUTINE_F3 COMMENT SHOWER: 클릭 시에도, F2(F3)과 같이 링크 버튼을 보여줌 */
		let remark9, http9 = getUpperRemarkLine(textarea1, currentLine); // rems=[a,b,c] 배열.
		removeLinkButton("prevComment");
		generateLinkButton(http9, "prevComment");
...
window.addEventListener("keydown", (e) => {
	...
	if (e.key === "F2") {
		...
		for (let i=backFoo4; i <= nextFoo4; i++) {
...
/** (F9로 만들어진) 각 줄로 가는 버튼을 주어진 DIV에 더한다 
보통은 def4.
*/
function addButtonToDiv(divId, buttonName, line1) {
  ...
  buttonElement.addEventListener("click", function() {
    gotoLine3(line1);  // Call the gotoLine2 function with the button name
  });
...
e.g. printFormat(2451,"이 이미지 흐린가? {0}", bBlur);
*/
